...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['convert'])) {
    if ($override['eligible']) {
        ...
        if ($check->num_rows === 0) {
            ...
            credit_earnings($conn, $uid, $override['override_amount'], 'town_override', 'town_override', null,
                'Town override for ' . date('F Y') . ' (2% of ' . format_currency($override['total_orders']) . ' from ' . $override['town'] . ')');

            flash_message('success', 'Town override of ' . format_currency($override['override_amount']) . ' added to your earnings!');
        } else {
                    ...
                    <div class="card-body">
                        ...
                        <?php if ($override['eligible']): ?>
                            ...
                            <div class="alert alert-success text-white">
                                ...
                                Town override already claimed to your earnings for this month!
                            </div>
                            <?php else: ?>
                            <form method="POST">
                                <button type="submit" name="convert" value="1" class="btn bg-gradient-success w-100"
                                        onclick="return confirm('Claim <?php echo format_currency($override['override_amount']); ?> town override to your earnings?')">
                                    <i class="material-icons">store</i> Claim <?php echo format_currency($override['override_amount']); ?> to Earnings
                                </button>
                            </form>
                            <?php endif; ?>
                        <?php else: ?>
